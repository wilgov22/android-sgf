 ...
 * 
 */
public class PostMovementsService extends IntentService {
	private Handler handleResult;
	private final String TAG = "SGF_PostMovementsService";

	public PostMovementsService() {
		super("PostMovementService");
		...
		handleResult = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				CharSequence contentText = "Movements Posted to Server";
				...
				if (!contentText.toString().contains("Movements Synchronized")) {
		...
		for (com.sgf.entities.Account account : accounts) {
			...
			if (!account.get_id().equals("1")) {
				...
				if (movementsTOSynch.size() > 0) {
					...
					Log.d(TAG,
							"INVOKE API WITH MovementArray ="
									+ object.toString());
					Map<String, String> params = new HashMap<String, String>();
					params.put("content", object.toString());
					POSTMovement postmovs = new POSTMovement(username,
							password, account.getSgf_account_id(), null, null,
							params, getApplicationContext());
					Message msg = postmovs.run();
					Message messageFinal = persistDataOnBD(msg);
					handleResult.sendMessage(messageFinal);
				}

			}
		}
		/*
		 * if(movementsSynched && this.handleResult!=null){ Bundle bundle=new
		 * Bundle(); bundle.putString("message",
		 * "Your new movements were synched"); Message
		 * msg=handleResult.obtainMessage(); msg.setData(bundle);
	...
	private Message persistDataOnBD(Message msg) {
		...
		// SUCESSO
		// responseInJson =
		// "{\"movement\":[{\"_id\":13,\"mov_id\":31,\"mov_account_id\":4,\"mov_type_id\":2,\"mov_cat_id\":6,\"mov_sub_cat_id\":null,\"mov_amount\":150.0,\"mov_desc\":\"we\",\"mov_date\":\"15-08-2012\",\"status_code\":\"C000\"}], \"status\":{\"status_code\":\"C000\"}}";
		// ERRO C999
		...
		if (!bundle.containsKey("RESPONSE")) {
			bundle.putString("RESPONSE", "Movements Synchronized");
		}
